  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Windows GUID Generator</h1>
        <p className="text-gray-600 dark:text-gray-400">Generate Windows Global Unique Identifiers (GUIDs)</p>
      </div>
        ...
        {guids.length > 0 && (
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-gray-900 dark:text-white">Generated GUIDs</h3>
              {guids.length > 1 && (
                <button
                  onClick={copyAll}
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {guids.map((guid, index) => (
                ...
                >
                  <code className="flex-1 text-sm font-mono text-gray-900 dark:text-gray-100 break-all">{formatGUID(guid)}</code>
                  <button
                    onClick={() => copyToClipboard(formatGUID(guid), index)}
                    className="ml-3 p-2 text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg transition-colors"
                  >
                    {copied === index ? (
                      <Check className="w-4 h-4" />
                    ) : (
      ...
      <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <h3 className="font-semibold text-blue-900 dark:text-blue-300 mb-2">About GUIDs</h3>
