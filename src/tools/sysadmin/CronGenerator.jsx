  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-white">Cron Expression Generator</h1>
        <p className="text-gray-400">Generate cron job expressions</p>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 space-y-4">
        <div className="grid grid-cols-5 gap-4">
          {[
            ...
            <div key={i} className="space-y-2">
              ...
              <input type="text" value={field.value} onChange={(e) => field.set(e.target.value)} className="input-field bg-gray-700 border-gray-600 text-white font-mono text-center" />
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {examples.map((ex, i) => (
              <button key={i} onClick={() => loadExample(ex.expr)} className="w-full text-left p-3 bg-gray-50 bg-gray-700 rounded-lg hover:hover:bg-gray-600">
                <code className="font-mono text-sm text-primary-400">{ex.expr}</code>
                <span className="ml-3 text-sm text-gray-400">{ex.desc}</span>
              </button>
            ))}
          </div>
        </div>
